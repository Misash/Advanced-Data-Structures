...
class RTree {
 public:
  ...
  struct Node {
    ...
    SpatialObject entry[M]; //maximo de rectangulos
    size_t size = 0;
  };
...
void RTree<N, ElemType, M, m>::insert(const Rectangle<N> &box,
  ...
  //si ha insertado limpiamente has nada
  if (!splitted_node) {
    return;
  }
...
template <size_t N, typename ElemType, size_t M, size_t m>
std::shared_ptr<typename RTree<N, ElemType, M, m>::Node> //node ptr
RTree<N, ElemType, M, m>::choose_leaf(const std::shared_ptr<Node> &current_node,
                                      const Rectangle<N> &box,
                                      const ElemType &value) {
  if (!current_node->is_leaf()) {
    SpatialObject *entry;
    std::shared_ptr<Node> next_node = choose_node(current_node, box, entry); //selecciona el mejor nodo
...
template <size_t N, typename ElemType, size_t M, size_t m>
std::shared_ptr<typename RTree<N, ElemType, M, m>::Node> //retorna un puntero
RTree<N, ElemType, M, m>::choose_node(const std::shared_ptr<Node> &current_node,
                                      const Rectangle<N> &box,
                                      SpatialObject *&entry) {

    //inicializa el minimo
  float minimum_area = (*current_node)[0].box.get_area();

  //copia de box
  Rectangle<N> enlarged_box = (*current_node)[0].box;

  //ajustar con el box ingresado ajusta los limites para hacer
  // un rectabuloa mas grande osea el box alargado
  ...
  //apunta al primer hijo
  std::shared_ptr<Node> node = (*current_node)[0].child_pointer;

  //la entrada del primer hijo la copia al entry  q es un espacio vectorial
